# Flux v2 on AKS https://github.com/gbaeke/quick-guides/blob/main/fluxv2/README.md

## Requirements https://www.youtube.com/watch?v=w_eoJbgDs3g
#flux sample https://github.com/jonathan-innis/go-example-app.git
##https://www.youtube.com/watch?v=60jhYD2wLkY adding a secret
...
az connectedk8s connect --name $AZ_KUBERNETES_CLUSTER --resource-group $AZ_RESOURCE_GROUP
az connectedk8s list --resource-group $AZ_RESOURCE_GROUP --output table

## Create a new Azure key vault
az keyvault create -n $AZ_KEY_VAULT -g $AZ_RESOURCE_GROUP -l eastus --enable-rbac-authorization

## Update an existing Azure key vault
az keyvault update -n $AZ_KEY_VAULT -g $AZ_RESOURCE_GROUP -l eastus --enable-rbac-authorization
az keyvault update -n $AZ_KEY_VAULT -g $AZ_RESOURCE_GROUP -l eastus --enable-rbac-authorization
...
az k8s-configuration flux create -g $AZ_RESOURCE_GROUP -c $AZ_KUBERNETES_CLUSTER \
